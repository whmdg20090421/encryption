...
import androidx.compose.foundation.tooltip.Tooltip
...
fun EncryptionProgressIcon(
    ...
    // Tooltip 内容
    val tooltipMessage = remember(stats) {
        buildTooltipMessage(stats)
    }

    TooltipBox(
        positionProvider = TooltipDefaults.rememberPlainTooltipPositionProvider(),
        tooltip = {
            Tooltip(text = { androidx.compose.material3.Text(tooltipMessage) })
        },
        state = tooltipState
    ) {
        Canvas(
 ...
 * 构建 Tooltip 消息
 */
private fun buildTooltipMessage(stats: SizeStats): String {
    return buildString {
